...
class Trail {
    ...
    walk(fn) {
        return this._path.map(fn);
    }
    splitInstructionList(instructions) {
        return instructions.split(",");
    }
    splitInstruction(instruction) {
        let instructionParts = instruction.split("");
        const direction = instructionParts.shift();
        return {
            "direction": direction,
            "steps": Number(instructionParts.join(""))
        };
    }
    ...
    shortestIntersectionDistance(t) {
        return this.intersections(t).reduce((acc, cur) => Math.min(acc, cur.distanceFrom()), Infinity);
    }
    fewestIntersectionSteps(t) {
        let stepsList = this.intersections(t).map(p => this.stepsToPoint(p) + t.stepsToPoint(p));
        return stepsList.reduce((acc, cur) => Math.min(acc, cur), Infinity);
    }
    toString() {
        return this.walk(x => x.toString()).join("->");
    }
}
